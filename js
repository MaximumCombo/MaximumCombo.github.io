...
function initializePage(modules, initIcons, initDocDirs, initDisplays, initFilters, initSelectables, souvenirAttributes, iconSpriteMd5) {
    ...
    var filter = {};
    try {
        filter = JSON.parse(lStorage.getItem('filters') || '{}') || {};
    }
    catch (exc) { }
    ...
    try {
        preferredManuals = JSON.parse(lStorage.getItem('preferredManuals') || '{}') || {};
    }
    catch (exc) { }
    var preferredLanguages = {};
    try {
        preferredLanguages = JSON.parse(lStorage.getItem('preferredLanguages') || '{}') || {};
    }
    catch (exc) { }
    ...
    try {
        displayOptions = JSON.parse(lStorage.getItem('display')) || defaultDisplayOptions;
    }
    catch (exc) { }
    ...
    try {
        searchOptions = JSON.parse(lStorage.getItem('searchOptions')) || defaultSearchOptions;
    }
    catch (exc) { }
        ...
        switch (newView) {
            case 'list': {
                ...
                for (var i = 0; i < modules.length; i++) {
                    ...
                    tr.appendChild(td2);
                    let infos = el("div", "infos", el("div", "inf-type inf", mod.Type), el("div", "inf-origin inf inf2", mod.Origin));
                    ...
                    infos.append(el("div", "inf-author inf", mod.Author), el("div", "inf-published inf inf2", mod.Published));
                    if (mod.TwitchPlaysSupport === 'Supported') {
                        ...
                        infos.append(el("div", "inf-twitch inf inf2", { title: mod.TwitchInfo }, mod.TwitchPlaysSpecial ? 'S' : mod.TwitchPlaysScore === undefined ? '' : mod.TwitchPlaysScore));
                    }
            ...
            case 'periodic-table': {
                ...
                for (let i = 0; i < modules.length; i++) {
                    ...
                    let div = el('a', `module ${mod.ExpertDifficulty}`, el('div', `symbol ${mod.DefuserDifficulty}`, mod.Symbol || '??', el('div', 'icon', { style: `background-image:url(iconsprite/${iconSpriteMd5});background-position:-${mod.X * 32}px -${mod.Y * 32}px` })), el('div', 'name', el('div', 'inner', mod.Name)), el('div', 'tpscore', mod.TwitchPlaysScore || ''), el('div', 'souvenir', souvenirStatuses[(mod.Souvenir && mod.Souvenir.Status) || 'Unexamined']), manualSelector);
            ...
            if (isMobileOpt) {
                ...
                for (let ix = 0; ix < initSelectables.length; ix++) {
                    ...
                    if (sel.PropName === 'manual' || !sel.ShowIconFunction(mod, mod.Manuals))
                        continue;
                    var iconDiv = el('div', 'icon', el('a', 'icon-link', { href: sel.UrlFunction(mod, mod.Manuals) }, el('img', 'icon-img', { src: sel.Icon }), el('span', 'icon-label', sel.HumanReadable)));
    ...
    for (const language of languages) {
        languagesOption.appendChild(el("div", null, el("input", "language-toggle", { type: "checkbox", name: "language", id: `lang-${language}`, "data-lang": language }), el("label", null, { for: `lang-${language}` }, language)));
    }
    ...
    $('input#profile-file').change(function () { const files = document.getElementById('profile-file').files; if (files.length === 1) {
        setProfile(files[0]);
    } });
